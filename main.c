...
void definir_velocidade_ventoinha(float porcentagem) {
    if (porcentagem > 100.0f) porcentagem = 100.0f;
...
float calcular_controle_pi(float temperatura_atual) {
    ...
    if (termo_integral > INTEGRAL_MAX) termo_integral = INTEGRAL_MAX;
    if (termo_integral < INTEGRAL_MIN) termo_integral = INTEGRAL_MIN;
...
void aplicar_controle(float temperatura_atual) {
    ...
    if (angulo_alvo > 180.0f) angulo_alvo = 180.0f;
    if (angulo_alvo < 0.0f) angulo_alvo = 0.0f;
...
void verificar_nova_temperatura_serial(void) {
    ...
    while ((caractere = getchar_timeout_us(0)) != PICO_ERROR_TIMEOUT) {
        ...
        if (caractere == '\n' || caractere == '\r') { // Se o usuário apertou Enter
            if (contador_chars > 0) {
